...
class Master extends DBConnection
{
	...
	function save_appointment()
	{
		...
		if (!((strtotime($sched_time) >= strtotime($morning_start) && strtotime($sched_time) <= strtotime($morning_end)) || (strtotime($sched_time) >= strtotime($afternoon_start) && strtotime($sched_time) <= strtotime($afternoon_end)))) {
			$resp['status'] = 'failed';
			$resp['msg'] = "Selected Schedule Time is Fully Booked.";
			return json_encode($resp);
			exit;
		}
		...
		if (empty($client_id))
			$sql = "INSERT INTO `clients` set name = '{$name}'  ";
		else
			$sql = "UPDATE `clients` set name = '{$name}' where id = '{$id}'  ";
		$save_inv = $this->conn->query($sql);
		$this->capture_err();
		if ($save_inv) {
			$client_id = (empty($client_id)) ? $this->conn->insert_id : $client_id;
			if (empty($id))
			...
			$sql = "INSERT INTO `patient_meta` (client_id,meta_field,meta_value) VALUES $data ";
			$this->conn->query("DELETE FROM `patient_meta` where client_id = '{$client_id}'");
	...
	function multiple_action()
	{
		...
		if ($_action != 'delete') {
			...
			$sql = "UPDATE `appointments` set status = '{$status}' where client_id in (" . (implode(",", $ids)) . ") ";
			$process = $this->conn->query($sql);
			$this->capture_err();
		} else {
			$sql = "DELETE s.*,i.*,im.* from  `appointments` s inner join `clients` i on s.client_id = i.id  inner join `patient_meta` im on im.client_id = i.id where s.client_id in (" . (implode(",", $ids)) . ") ";
			$process = $this->conn->query($sql);
			$this->capture_err();
		}
		if ($process) {
